...
opts_chunk$set(echo=FALSE, message=FALSE, warning=FALSE, cache=TRUE,
  dev="pdf", out.width = "50%", fig.pos="hbt", fig.align="center")
```
...
```{r histvb, fig.cap="Log scale distribution of the 864 estimated values of virgin biomass. The red line show the value returned by the base case model run."}
ggplot(res, aes(TotBio_Unfished)) + geom_histogram(bins=40) + scale_x_log10() +
  xlab("Virgin biomass (t)") + ylab("") +
  geom_vline(aes(xintercept=c(stock(sa)[,1])), colour='red')
```
...
```{r plotcond, fig.cap="Boxplot of values for virgin biomass estimated by models with each of the three scaling scenarios (area, biomass and catch) and CPUE series (Japanese only, jap, Japanese and Portuguese, jappt, and Taiwanese and Portuguese, twnpt. The red line shows the value returned by the case base run."}
ggplot(res, aes(x=as.factor(llq), y=TotBio_Unfished)) + geom_boxplot() + scale_y_log10() +
  facet_grid(cpue ~ scaling) + geom_hline(aes(yintercept=c(stock(sa)[,1])), colour='red') +
  xlab("LL catchability") + ylab("Virgin biomass (t)")
```
...
```{r compare, fig.cap="Estimates of virgin biomass (in log scale) vs. current (2015) depletion levels for all 864 model runs."}
ggplot(res, aes(x=TotBio_Unfished, y=SPB_2015/SPB_1950)) + geom_point() + scale_x_log10()+
  ylab(bquote(SSB[2015] / "Virgin biomass")) + xlab("Virgin biomass (t)")
```
...
```{r filterom, fig.cap="Population trajectories (recruitment, SSB, catch and F) estimated by the filtered OM grid for Indian Ocean swordfish, containing only runs in which $B_0 < 1,5 Mt$. The black line shows the median value, while the darker and lighter ribbons show the 50\\% and 90\\% quantiles, respectively."}
idx <- res$TotBio_Unfished < 1.5e6
oms <- iter(oms, idx)
plot(oms)
```
